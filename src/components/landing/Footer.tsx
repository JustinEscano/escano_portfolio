"use client";

import React from 'react';

const Footer: React.FC = () => {
  return (
    <footer className="bg-primary text-gray-400 py-16 border-t border-white/5">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 md:px-10 lg:px-12">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-24 justify-between">
          
          {/* Logo & Info column */}
          <div className="lg:w-1/3 xl:w-1/4 space-y-6 max-w-sm">
            <div className="flex items-center gap-3">
              <h2 className="text-3xl font-bold text-white tracking-tight">Justin Escano</h2>
              <span className="inline-block w-2.5 h-2.5 rounded-full bg-accent shadow-[0_0_8px_rgba(59,130,246,0.5)]"></span>
            </div>
            <p className="text-gray-500 leading-relaxed text-[15px] pr-4">
              Full Stack Developer crafting digital experiences with passion and precision.
            </p>
            <div className="flex gap-4 pt-2 mb-8 lg:mb-0">
              <a href="https://github.com/JustinEscano" target="_blank" rel="noopener noreferrer" className="w-10 h-10 border border-white/10 bg-secondary hover:bg-white/10 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0016 8c0-4.42-3.58-8-8-8z"/></svg>
              </a>
              <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="w-10 h-10 border border-white/10 bg-secondary hover:bg-white/10 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 01.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z"/></svg>
              </a>
            </div>
          </div>

          {/* Nav Links & Grids */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 sm:gap-12 flex-1 pt-8 lg:pt-0">
            
            {/* Navigation Block */}
            <div className="space-y-6">
              <h3 className="text-white font-bold tracking-widest uppercase text-sm mb-6">Navigation</h3>
            <ul className="space-y-4">
              <li><a href="#home" className="hover:text-accent transition-colors">Home</a></li>
              <li><a href="#projects" className="hover:text-accent transition-colors">Projects</a></li>
              <li><a href="#about" className="hover:text-accent transition-colors">About</a></li>
              <li><a href="#contact" className="hover:text-accent transition-colors">Contact</a></li>
            </ul>
          </div>

          {/* Projects Links */}
          <div className="space-y-6">
            <h3 className="text-white font-bold tracking-widest uppercase text-sm mb-6">Projects</h3>
            <ul className="space-y-4">
              <li><a href="#" className="hover:text-accent transition-colors">WATCHOUT!</a></li>
              <li><a href="#" className="hover:text-accent transition-colors">PALDEX</a></li>
              <li><a href="#" className="hover:text-accent transition-colors">U-CASH</a></li>
              <li><a href="#" className="hover:text-accent transition-colors">PetPal</a></li>
              <li><a href="#" className="hover:text-accent transition-colors">Baskit</a></li>
              <li><a href="#" className="hover:text-accent transition-colors">Cryphoria</a></li>
            </ul>
          </div>

          {/* Skills & Connect */}
          <div className="space-y-12">
            <div>
              <h3 className="text-white font-bold tracking-widest uppercase text-sm mb-6">Skills</h3>
              <ul className="space-y-4">
                <li className="hover:text-accent transition-colors cursor-pointer">Frontend</li>
                <li className="hover:text-accent transition-colors cursor-pointer">Backend</li>
                <li className="hover:text-accent transition-colors cursor-pointer">Databases</li>
              </ul>
            </div>
            <div className="hidden lg:block">
              <h3 className="text-white font-bold tracking-widest uppercase text-sm mb-6">Connect</h3>
              <ul className="space-y-4">
                <li><a href="https://github.com/JustinEscano" target="_blank" rel="noopener noreferrer" className="hover:text-accent transition-colors">GitHub</a></li>
                <li><a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="hover:text-accent transition-colors">LinkedIn</a></li>
              </ul>
            </div>

          </div>
        </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
